...
class SRTD_bias:
    ...
    def AGD_optimization (self, seed=None):
        ...
        for t in range(self.max_iter):
            ...
            rmseloss,reg_loss=self.RMSE_loss()
            deltaLog = (rmseloss-lastLog)/np.abs(lastLog)
            ...
            lastLog = rmseloss
            print('Iteration ' + str(t+1) + ' Error: '+str(rmseloss)+'Reg Error: '+str(reg_loss))
    
    def RMSE_loss(self):
        P = np.empty((self.num_targets,self.num_drugs,self.num_drugs))
        ...
        for i in range(self.num_targets):
            for j in range(self.num_drugs):
                for k in range(self.num_drugs):
                    P[i,j,k] = np.matmul(self.V[i]*self.U[j],self.U[k].T)
                    P[i,j,k] += self.B_u[j] + self.B_u[k]
        P_vec = P.flatten()[np.where(self.mask.flatten())]
        Y_vec = self.Y.flatten()[np.where(self.mask.flatten())]
        error = mean_squared_error(Y_vec,P_vec)

        reg_error = 0
        if (type(self.drugSim)!=type(None)):
            S_d = self.drugSim - np.matmul(self.U,self.U.T)
            reg_error += self.lambda_sd*np.linalg.norm(S_d,ord='fro')
        if (type(self.targetSim)!=type(None)):
            S_t = self.targetSim - np.matmul(self.V,self.V.T)
            reg_error += self.lambda_st*np.linalg.norm(S_t,ord='fro')
        reg_error += self.lambda_d*(np.linalg.norm(self.U,ord='fro'))+self.lambda_t*(np.linalg.norm(self.V,ord='fro'))
...
def get_data(cells,drugs,dataset="MERCK_deepsynscores"):
    data = []
    mask = []
    for cell in cells:
...
def get_new_V(gene_exprs,train_cells,test_cells,model,n_neighbors):
    knn = NearestNeighbors(n_neighbors=n_neighbors)
    train_exprs = gene_exprs.loc[train_cells]
    print(train_exprs)
    knn.fit(train_exprs)
    new_V = []
    for cell in test_cells:
        neighbors = knn.kneighbors(np.array([gene_exprs.loc[cell]]),return_distance=False)[0]
        V_pred = np.mean(model.get_V()[neighbors])
        new_V.append(V_pred)
...
if __name__=="__main__":
    ...
    hyperparameters=pd.read_csv("Data/MERCK_results/SRTD_KNN/SRTD_KNN_"+str(outer_fold)+"_"+str(inner_fold)+".csv",index_col=0)
    ...
    for i,j in hyperparameters.loc[hyperparameters['AUPR'].isnull()].iterrows():
        ...
        lambda_sd=j[5]
        n_neighbors = int(j[6])
        model=SRTD_bias(num_factors=num_factors,theta=theta,lambda_d=lambda_d,lambda_t=lambda_t,lambda_sd=lambda_sd,lambda_st=lambda_st,max_iter=100)
        model.fix_model(train_mask,train_data,targetSim=cell_sim.to_numpy(),drugSim=drug_sim.to_numpy())
        new_V = get_new_V(gene_expression,train_cells,val_cells,model,n_neighbors)
        y_val = val_data.flatten()[np.where(val_mask.flatten())]
        y_pred = model.predict_new(new_V).flatten()[np.where(val_mask.flatten())]
        precision, recall, thresholds= precision_recall_curve(y_val>30,y_pred)
        hyperparameters.loc[i,'AUPR']=auc(recall,precision)
        hyperparameters.to_csv("Data/MERCK_results/SRTD_KNN/SRTD_KNN_"+str(outer_fold)+"_"+str(inner_fold)+".csv")
