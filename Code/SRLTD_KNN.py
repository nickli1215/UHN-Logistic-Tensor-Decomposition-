...
if __name__=="__main__":
    ...
    hyperparameters=pd.read_csv("Data/MERCK_results/SRLTD_KNN/SRLTD_KNN_"+str(outer_fold)+"_"+str(inner_fold)+".csv",index_col=0)
    ...
    for i,j in hyperparameters.loc[hyperparameters['AUPR'].isnull()].iterrows():
        ...
        lambda_sd=j[5]
        n_neighbors = int(j[6])
        model=SRLTD_bias(num_factors=num_factors,theta=theta,lambda_d=lambda_d,lambda_t=lambda_t,lambda_sd=lambda_sd,lambda_st=lambda_st,max_iter=150)
        ...
        hyperparameters.to_csv("Data/MERCK_results/SRLTD_KNN/SRLTD_KNN_"+str(outer_fold)+"_"+str(inner_fold)+".csv")