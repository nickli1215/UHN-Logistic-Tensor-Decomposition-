...
if __name__=="__main__":
    ...
    hyperparameters=pd.read_csv("Data/MERCK_results/SRTD_"+str(outer_fold)+"_"+str(inner_fold)+".csv",index_col=0)
    gene_expression=pd.read_csv("Data/deepsynergy_gene_expression.csv",index_col=0)
    data=[]
    for i,j in hyperparameters.loc[pd.isnull(hyperparameters['MSE'])].sample(n=hyperparameters.loc[pd.isnull(hyperparameters['MSE'])].shape[0]).iterrows():
        num_factors = int(j[0])
        theta= j[1]
        lambda_d=j[2]
        lambda_t=j[3]
        ...
        hyperparameters.loc[i,'MSE']=mean_squared_error(y_val,y_pred)
        hyperparameters.to_csv("Data/MERCK_results/SRTD_"+str(outer_fold)+"_"+str(inner_fold)+".csv")
